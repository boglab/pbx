...
while line:
    ...
    if alength > (slen / 3) / 2 and pident >= 60:
        if seen_at_start == False and (qstart <= buffer_len or qend <= buffer_len):
            sys.stderr.write("Warning: Contig %s (%d bp) has a TAL effector repeat at %d near sequence boundary. Assembly of this region is likely incomplete." % (qname, qlen, qstart) + "\n")
            seen_at_start = True
        
        if seen_at_end == False and (qstart >= (qlen - buffer_len) or qend >= (qlen - buffer_len)):
            sys.stderr.write("Warning: Contig %s (%d bp) has a TAL effector repeat at %d near sequence boundary. Assembly of this region is likely incomplete." % (qname, qlen, qstart) + "\n")
            seen_at_end = True
